...
- After connecting, sync fetches the WhatsApp `regular_high` app-state delta so starred-message and delete-for-me changes made while `wacli` was offline are caught up instead of relying only on live push notifications.
